...
/// <summary>
/// #1851's probe-failure contract for the PAYLOAD path, Lite half. #1837 gave the ENUMERATING collectors a
/// channel for items they could not probe; a collector whose one result set IS the payload had none, so
...
///
/// <para>
/// The declaration (<see cref="ICollectorDefinition{TRow}.EmitsProbeFailures"/>) is what makes that read
/// safe, and is the difference from #1837: an enumeration's first result set is a bare item list, so
/// anything after it can only be failures, while a payload reader may legitimately hold sets the definition
/// itself consumes (<c>tempdb_stats</c> reads two). Nothing may advance a non-declaring collector's reader.
/// </para>
///
/// <para>
/// Behavioral throughout — a <see cref="DataSet"/> supplies a real multi-result-set <c>DbDataReader</c>, so
/// the collector's own read and the trailing read are exercised against each other rather than described.
/// Darling.Tests' <c>DarlingPayloadProbeFailureTests</c> pins the identical expectations against the
/// identical shared methods, so editing one app's copy alone fails a build.
/// </para>
/// </summary>
public sealed class PayloadProbeFailureTests
{
    ...
    {
        /* Every payload collector that shipped before #1851 returns one result set. Reading it must produce
           no failures and NO note, or this contract would annotate every healthy cycle in the product. */
    ...
    public async Task An_Empty_Trailing_Result_Set_Is_The_Same_As_None()
    {
        /* The shape database_size_stats actually returns on a healthy server: the failure table is declared
           and selected unconditionally, so the trailing set is present but empty. Indistinguishable from a
           collector that never had one — which is also what makes the Azure variant, whose cursor-less query
           emits no such set at all, safe to declare the contract. */
    ...
    public async Task The_Empty_Enumeration_Breadcrumb_Never_Rides_A_Payload_Note()
    {
        /* A payload collector has no enumeration whose emptiness could be reported, and its own empty result
           set is already visible as rows_collected = 0. Composing the enumeration breadcrumb here would put
           "enumeration yielded 0 items" on a collector that never enumerated for the host at all. */
    ...
    public async Task Null_Item_And_Error_Text_Do_Not_Drop_The_Failure()
    {
        /* ERROR_MESSAGE() should never be NULL, but a probe failure is the LAST thing that may vanish for
           want of its own text — the count is the part the operator acts on. The two unnamed cases stay
           distinguishable for the reason #1837 separated them: a malformed set sends someone hunting a SQL
           defect, and a genuine unnamed failure must not. */
        var dataSet = new DataSet();
    ...
    public void DatabaseSizeStats_OnPrem_Records_What_Its_Cursor_Could_Not_Probe()
    {
        /* #1851's defect, closed: the per-database CATCH was empty, so a database that was mid-restore or
           inaccessible contributed no used_size_mb under a SUCCESS row that said nothing had happened. The
           SQL is one long const, so this is a text pin — the live behavior needs a server that denies a
           database. */
    ...
    public void Host_Reads_The_Trailing_Set_After_The_Payload_And_Before_The_Write()
    {
        ...
        var write = plainPath.IndexOf("WriteBatch(duckConnection", StringComparison.Ordinal);
